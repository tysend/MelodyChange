...
# -*- coding: utf-8 -*-
"""
This experiment was created using PsychoPy2 Experiment Builder (v1.83.04), Wed May 25 08:34:48 2016
...
practice_trial_text = visual.TextStim(win=win, ori=0, name='practice_trial_text',
    text='Hit the space bar every time you hear the melody change.\nIf you can anticipate or predict the melody change, hit the space bar at the time you expect the melody to change.\n\nWhen you are ready to go to the actual test, hit the down cursor key.',    font='Georgia',
...
Ready_text = visual.TextStim(win=win, ori=0, name='Ready_text',
    text='If you are ready, hit the space bar to start the experiment.',    font='Georgia',
    pos=[0, 0], height=0.1, wrapWidth=None,
    color='white', colorSpace='rgb', opacity=1,
    depth=0.0)
...
During_trial_text = visual.TextStim(win=win, ori=0, name='During_trial_text',
    text='Hit the space bar every time you hear the melody change.\nIf you can anticipate or predict the melody change, hit the space bar at the time you expect the melody to change.',    font='Georgia',
    pos=[0, 0.5], height=0.1, wrapWidth=None,
    color='white', colorSpace='rgb', opacity=1,
    depth=-1.0)
sound_1 = sound.Sound('A', secs=-1)
sound_1.setVolume(1)
go_next_text = visual.TextStim(win=win, ori=0, name='go_next_text',
    text='Ready to start the next trial?\nHit the space bar.',    font='Georgia',
    pos=[0, 0], height=0.1, wrapWidth=None,
    color='white', colorSpace='rgb', opacity=1,
    depth=-4.0)
...
bye_text = visual.TextStim(win=win, ori=0, name='bye_text',
    text='Many thanks for participating in the "Melody Change Detection" experiment!\nYou are the best!',    font='Georgia',
    pos=[0, 0], height=0.1, wrapWidth=None,
    color='white', colorSpace='rgb', opacity=1,
    depth=0.0)
...
InstructionComponents.append(Inst_text)
...
while continueRoutine:
    ...
    if t >= 0.0 and Inst_text.status == NOT_STARTED:
        ...
        Inst_text.tStart = t  # underestimates by a little under one frame
...
for thisPractice_trial in practice_trials:
    ...
    go_test_resp = event.BuilderKeyResponse()  # create an object of type KeyResponse
    go_test_resp.status = NOT_STARTED
    # keep track of which components have finished
    practice_trialComponents = []
    practice_trialComponents.append(practice_trial_text)
    practice_trialComponents.append(practice_sound)
    practice_trialComponents.append(go_test_resp)
    ...
    while continueRoutine:
        ...
        if t >= 0.0 and practice_trial_text.status == NOT_STARTED:
            # keep track of start time/frame for later
            practice_trial_text.tStart = t  # underestimates by a little under one frame
        ...
        if t >= 5.0 and go_test_resp.status == NOT_STARTED:
            # keep track of start time/frame for later
            go_test_resp.tStart = t  # underestimates by a little under one frame
...
frameN = -1
# update component parameters for each repeat
go_test = event.BuilderKeyResponse()  # create an object of type KeyResponse
go_test.status = NOT_STARTED
# keep track of which components have finished
Ready_for_testComponents = []
Ready_for_testComponents.append(Ready_text)
Ready_for_testComponents.append(go_test)
...
while continueRoutine:
    ...
    if t >= 0.0 and go_test.status == NOT_STARTED:
        # keep track of start time/frame for later
        go_test.tStart = t  # underestimates by a little under one frame
...
for thisTrials_loop in trials_loop:
    ...
    detection_resp = event.BuilderKeyResponse()  # create an object of type KeyResponse
    detection_resp.status = NOT_STARTED
    go_next_resp = event.BuilderKeyResponse()  # create an object of type KeyResponse
    go_next_resp.status = NOT_STARTED
    # keep track of which components have finished
    trialComponents = []
    trialComponents.append(ISI)
    trialComponents.append(During_trial_text)
    trialComponents.append(sound_1)
    trialComponents.append(detection_resp)
    trialComponents.append(go_next_text)
    trialComponents.append(go_next_resp)
    ...
    while continueRoutine:
        ...
        if t >= 1.0 and During_trial_text.status == NOT_STARTED:
            # keep track of start time/frame for later
            During_trial_text.tStart = t  # underestimates by a little under one frame
            During_trial_text.frameNStart = frameN  # exact frame index
            During_trial_text.setAutoDraw(True)
        if During_trial_text.status == STARTED and t >= (105.0-win.monitorFramePeriod*0.75): #most of one frame period left
        ...
        if sound_1.status == STARTED and t >= (5.0 + (100.0-win.monitorFramePeriod*0.75)): #most of one frame period left
            sound_1.stop()  # stop the sound (if longer than duration)
        
        # *detection_resp* updates
        if t >= 5.0 and detection_resp.status == NOT_STARTED:
            # keep track of start time/frame for later
            detection_resp.tStart = t  # underestimates by a little under one frame
            ...
            event.clearEvents(eventType='keyboard')
        if detection_resp.status == STARTED and t >= (5.0 + (100.0-win.monitorFramePeriod*0.75)): #most of one frame period left
        ...
        if t >= 105.5 and go_next_text.status == NOT_STARTED:
            # keep track of start time/frame for later
            go_next_text.tStart = t  # underestimates by a little under one frame
        ...
        if t >= 105.5 and go_next_resp.status == NOT_STARTED:
            # keep track of start time/frame for later
            go_next_resp.tStart = t  # underestimates by a little under one frame
...
# keep track of which components have finished
good_byeComponents = []
good_byeComponents.append(bye_text)
...
while continueRoutine and routineTimer.getTime() > 0:
    ...
    if t >= 0.0 and bye_text.status == NOT_STARTED:
        # keep track of start time/frame for later
        bye_text.tStart = t  # underestimates by a little under one frame
        bye_text.frameNStart = frameN  # exact frame index
        bye_text.setAutoDraw(True)
    if bye_text.status == STARTED and t >= (0.0 + (1.0-win.monitorFramePeriod*0.75)): #most of one frame period left
